...
A **standalone** tool (not tied to the assistant): find the **engaging moments** in a long
video and clip just those — no timeline editing. Two ways:
...
It transcribes the audio (Whisper), detects where topics start/end, scores each for how
clip-worthy it is (hooks, stories, questions, strong takes, specifics), and cuts **only the
good ones** — skipping boring stretches, never mid-sentence, never touching the original.
No AI writes hooks or captions (AI titles are opt-in via `--titles`); `--even` gives uniform
5–7 min chunks instead. Needs `ffmpeg` and a Whisper backend on your Mac (or pass a
